...
export default function Home() {
    ...
    const addNotice = async (e) => { e.preventDefault(); try { let fileData = null; let fileType = null; if (noticeFile) { if (noticeFile.type !== 'application/pdf') { showToast("Only PDF files are allowed", "error"); return; } if (noticeFile.size > 2 * 1024 * 1024) { showToast("PDF too large (Max 2MB)", "error"); return; } fileData = null; } await addDoc(collection(db, "notices"), { text: noticeText, fileUrl: fileData, fileType: fileType, fileName: noticeFile ? noticeFile.name : null, createdAt: new Date().toISOString(), date: new Date().toLocaleDateString() }); setShowNoticeModal(false); setNoticeText(''); setNoticeFile(null); fetchNotices(); showToast("Notice Posted!", "success"); } catch (err) { showToast("Error posting notice", "error"); } };
    ...
    // === CHALLAN PDF (SINGLE PAGE, LANDSCAPE, 3 COPIES, STRETCHED TO FIT) ===
    const downloadPDF = (challan, profileData) => { 
        ...
        const drawCopy = (xOffset, copyTitle) => { 
            ...
            // Student Details (Gap Increased to 7mm for filling space)
            let y = 40; 
            ...
            // Table (Padding 3 to fill page without overflow)
            autoTable(doc, { 
                startY: y, 
                margin: { left: xOffset + 4 }, 
                tableWidth: colWidth - 8, 
                theme: 'grid', 
                pageBreak: 'avoid',
                head: [['Sr', 'Nature of Dues', 'Amount']], 
                body: [
                    [{ content: 'A) Fee structure per Semester', colSpan: 3, styles: { fillColor: [240, 240, 240], fontStyle: 'bold' } }], 
                    ['1', 'Tuition Fee', '200'], ['2', 'Admission Fee', '100'], ['3', 'Hostel Room Rent', roomRent], 
                    [{ content: 'Sub-Total (A)', colSpan: 2, styles: { fontStyle: 'bold' } }, { content: subTotalA, styles: { fontStyle: 'bold' } }], 
                    [{ content: 'B) College-Hostel Dues/Sem', colSpan: 3, styles: { fillColor: [240, 240, 240], fontStyle: 'bold' } }], 
                    ['1', 'Library/Dev/Utility Funds', '300'], ['2', 'Sports/Welfare/Exam Funds', '300'], 
                    [{ content: 'Grand Total (A+B)', colSpan: 2, styles: { fontStyle: 'bold', fontSize: 10 } }, { content: totalAmount, styles: { fontStyle: 'bold', fontSize: 10 } }]
                ], 
        ...
        drawCopy(0, "BANK COPY"); 
        drawCopy(colWidth, "COLLEGE COPY"); 
        drawCopy(colWidth * 2, "STUDENT COPY"); 
        
        // Dotted Lines
        doc.setDrawColor(0, 0, 0); 
        doc.setLineWidth(0.1); 
        doc.setLineDash([2, 2], 0); 
        doc.line(colWidth, 5, colWidth, pageHeight - 5); 
        doc.line(colWidth * 2, 5, colWidth * 2, pageHeight - 5); 
        
        doc.save(`Challan_${challan.challanNo}.pdf`); 
        showToast("Downloaded!", "success"); 
    };
    
    // === VERIFIED LIST PDF (Centered & No Date) ===
    const downloadBatchPDF = (batchName, studentsList) => { 
        ...
        autoTable(doc, { 
            startY: 30, 
            head: [['Sr', 'Name', 'F/Name', 'CNIC', 'Status', 'Paid']], 
            body: studentsList.map((r, i) => [i+1, r.name, r.fname, r.cnic, r.status, `Rs. ${r.amount}`]), 
            theme: 'grid', 
            headStyles: { fillColor: [0, 77, 0], halign: 'center' }, 
            bodyStyles: { halign: 'center' }, 
            styles: { halign: 'center' } 
        }); 

        doc.save(`${batchName}_Verified_List.pdf`); 
        showToast(`Downloaded ${batchName} List!`); 
    };
    ...
    if (isAdmin) {
        ...
        return (
            <div className="flex min-h-screen bg-[#f8f9fa] font-sans">
                <ToastPopup />
                <aside className="w-64 bg-[#004d00] text-white flex flex-col min-h-screen shadow-2xl flex-shrink-0">
                    <div className="p-6 text-center border-b border-green-800">
                        <div className="mb-3 flex justify-center"><img src={logo}  alt="Logo" className="h-16 w-auto object-contain" /></div>
                        <h2 className="text-lg font-bold">ADMIN PANEL</h2><p className="text-xs text-green-200">GECE MITHI</p>
                    </div>
                    <div className="flex-1 p-4 space-y-2">
                        <button onClick={() => setActiveTab('dashboard')} className={`flex items-center w-full px-4 py-3 font-bold rounded shadow ${activeTab === 'dashboard' ? 'bg-white text-[#004d00]' : 'text-green-100 hover:bg-[#005a00]'}`}><Icons.Grid /> <span className="ml-3">ALL STUDENTS</span></button>
                        <button onClick={() => setActiveTab('verifiedList')} className={`flex items-center w-full px-4 py-3 font-bold rounded shadow ${activeTab === 'verifiedList' ? 'bg-white text-[#004d00]' : 'text-green-100 hover:bg-[#005a00]'}`}><Icons.List /> <span className="ml-3">Challan LIST</span></button>
                        <button onClick={() => setActiveTab('noticeBoard')} className={`flex items-center w-full px-4 py-3 font-bold rounded shadow ${activeTab === 'noticeBoard' ? 'bg-white text-[#004d00]' : 'text-green-100 hover:bg-[#005a00]'}`}><Icons.Bell /> <span className="ml-3">NOTICE BOARD</span></button>
                    </div>
                ...
                <main className="flex-1 p-8 overflow-y-auto">
                    ...
                    {activeTab === 'dashboard' && (
                        <>
                            <h2 className="text-2xl font-bold text-[#004d00] mb-6">Student Management</h2>
                            <div className="bg-white rounded-lg shadow border border-gray-200 overflow-hidden">
                                {allStudents.map((student) => (
                                    <div key={student.cnic} className="border-b border-gray-100 last:border-0">
                                        <div className="bg-gray-50 px-6 py-3 flex justify-between items-center hover:bg-gray-100">
                                            <div><h3 className="font-bold text-gray-800 uppercase">{student.profile?.fullName || "Unknown Name"}</h3><p className="text-xs text-gray-500 font-mono">CNIC: {student.cnic}</p></div>
                                            <div className="flex items-center gap-3"><button onClick={() => setAdminChallanModal({show: true, cnic: student.cnic})} className="text-xs bg-[#004d00] text-white px-3 py-1.5 rounded flex items-center hover:bg-green-800"><Icons.Plus /> Add Challan</button><span className="text-xs font-bold bg-gray-200 text-gray-600 px-2 py-1 rounded">{student.challans?.length || 0} Records</span></div>
                                        </div>
                                        {student.challans && student.challans.length > 0 && (
                                            <div className="px-6 py-4 bg-white">
                                                <table className="w-full text-sm text-left">
                                                    <thead className="bg-[#004d00] text-white"><tr><th className="py-2 px-2">Challan No</th><th className="py-2 px-2">Amount</th><th className="py-2 px-2">Date</th><th className="py-2 px-2">Receipt</th><th className="py-2 px-2">Status</th><th className="py-2 px-2 text-right">Actions</th></tr></thead>
                                                    <tbody>
                                                        {student.challans.map((ch) => (
                                                            <tr key={ch.id} className="border-b last:border-0 hover:bg-gray-50">
                                                                <td className="py-3 px-2 font-mono text-gray-800 text-xs">{ch.challanNo}</td><td className="py-3 px-2 font-bold text-gray-800">Rs.{ch.amount}</td><td className="py-3 px-2 text-gray-500">{ch.date}</td>
                                                                <td className="py-3 px-2">{ch.receipt ? (<a href={ch.receipt} download={`Receipt_${ch.challanNo}`} className="text-blue-600 hover:underline flex items-center"><Icons.Eye /> View</a>) : <span className="text-gray-400">-</span>}</td>
                                                                <td className="py-3 px-2"><span className={`px-2 py-1 rounded text-xs font-bold ${ch.status === 'Verified' ? 'bg-green-100 text-green-700' : ch.status === 'Pending Verification' ? 'bg-orange-100 text-orange-600' : 'bg-red-100 text-red-600'}`}>{ch.status}</span></td>
                                                                <td className="py-3 px-2 text-right flex justify-end gap-2">
                                                                    {ch.status !== 'Verified' && (<button onClick={() => verifyChallan(student.cnic, ch.id)} title="Verify" className="p-1.5 bg-green-100 text-green-700 rounded hover:bg-green-200"><Icons.Check /></button>)}
                                                                    <button onClick={() => deleteChallan(student.cnic, ch.id)} title="Delete" className="p-1.5 bg-red-100 text-red-600 rounded hover:bg-red-200"><Icons.Trash /></button>
                                                                    <button onClick={() => downloadPDF(ch, student.profile)} title="Download PDF" className="p-1.5 bg-gray-100 text-gray-600 rounded hover:bg-gray-200"><Icons.Download /></button>
                                                                </td>
                                                            </tr>
                                                        ))}
                                                    </tbody>
                    ...
                    {/* VERIFIED LIST - BATCH WISE */}
                    {activeTab === 'verifiedList' && (
                        <>
                            <h2 className="text-2xl font-bold text-[#004d00] mb-6">Verified Students</h2>
                            
                            {batchKeys.length === 0 ? (
                                <div className="flex flex-col items-center justify-center py-20 bg-white rounded-lg border border-dashed border-gray-300 text-gray-400">
                                    <Icons.File />
                                    <p className="mt-3">No verified batches yet.</p>
                                    <p className="text-xs">Verify a student challan to generate a batch list automatically.</p>
                                </div>
                            ) : (
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                    {batchKeys.map(batch => (
                                        <div key={batch} className="bg-white rounded-lg shadow-md p-6 border-t-4 border-[#004d00] hover:shadow-lg transition">
                                            <h3 className="text-xl font-bold text-gray-800 mb-2">{batch}</h3>
                                            <p className="text-sm text-gray-500 mb-4">Total Verified: <span className="font-bold text-[#004d00]">{verifiedBatches[batch].length}</span></p>
                                            <button 
                                                onClick={() => downloadBatchPDF(batch, verifiedBatches[batch])}
                                                className="w-full bg-[#004d00] text-white py-2 rounded font-bold text-sm flex items-center justify-center hover:bg-green-800 transition"
                                            >
                            ...
                            <div className="mt-8 bg-white rounded-lg shadow border border-gray-200 overflow-hidden">
                                <div className="bg-gray-50 px-6 py-4 border-b border-gray-100"><h3 className="font-bold text-gray-700">All Verified Students</h3></div>
                                <table className="w-full text-sm text-left">
                                    <thead className="bg-gray-100 text-gray-600 uppercase font-bold text-xs"><tr><th className="px-6 py-3">Batch</th><th className="px-6 py-3">Name</th><th className="px-6 py-3">F/Name</th><th className="px-6 py-3">CNIC</th><th className="px-6 py-3">Status</th></tr></thead>
                                    <tbody className="divide-y divide-gray-100">
                                        {batchKeys.map(batch => (
                                            verifiedBatches[batch].map((student, idx) => (
                                                <tr key={`${batch}-${idx}`} className="hover:bg-gray-50">
                                                    <td className="px-6 py-3 font-bold text-[#004d00]">{batch}</td>
                                                    <td className="px-6 py-3">{student.name}</td>
                                                    <td className="px-6 py-3 text-gray-500">{student.fname}</td>
                                                    <td className="px-6 py-3 font-mono text-xs">{student.cnic}</td>
                                                    <td className="px-6 py-3"><span className="bg-green-100 text-green-800 text-xs px-2 py-1 rounded font-bold">Paid</span></td>
                                                </tr>
                                            ))
                                        ))}
                                    </tbody>
                    ...
                    {activeTab === 'noticeBoard' && (
                        <>
                            <div className="flex justify-between items-center mb-6">
                                <h2 className="text-2xl font-bold text-[#004d00]">Manage Notice Board</h2>
                                <button onClick={() => setShowNoticeModal(true)} className="bg-[#004d00] text-white px-5 py-2.5 rounded-lg font-bold shadow-lg hover:bg-green-900 transition flex items-center"><Icons.Plus /> <span className="ml-2">ADD NOTICE</span></button>
                            </div>
                            <div className="space-y-3">
                                {notices.map((n) => (
                                    <div key={n.id} className="bg-white p-4 rounded shadow-sm border-l-4 border-[#004d00] flex justify-between items-center">
                                        <div><p className="font-bold text-gray-800">{n.text}</p><p className="text-xs text-gray-400">{n.date}</p>{n.fileUrl && <a href={n.fileUrl} download={n.fileName} className="text-xs text-blue-600 hover:underline flex items-center mt-1"><Icons.Paperclip /> {n.fileName}</a>}</div>
                                        <button onClick={() => deleteNotice(n.id)} className="text-red-500 bg-red-50 p-2 rounded hover:bg-red-100"><Icons.Trash /></button>
                                    </div>
                                ))}
                            </div>
                        </>
    ...
    return (
        <div className="flex min-h-screen bg-[#f8f9fa] font-sans">
            <ToastPopup />
            <aside className="w-72 bg-[#004d00] text-white flex flex-col min-h-screen shadow-2xl z-20 flex-shrink-0">
                <div className="p-8 text-center border-b border-green-800 mb-4"><div className="bg-white w-20 h-20 mx-auto mb-3 rounded-full flex items-center justify-center shadow-lg p-2"><img src={logo} alt="GECE Logo" className="w-full h-full object-contain" /></div><h2 className="text-[11px] font-bold text-white tracking-widest leading-tight">{siteConfig.collegeName}</h2><h2 className="text-[11px] font-bold text-yellow-400 tracking-widest leading-tight mt-1">OF EDUCATION (M/W) MITHI</h2></div>
                <nav className="flex-1 px-4 space-y-3">
                    <button onClick={() => setActiveTab('dashboard')} className={`flex items-center w-full px-5 py-3.5 text-xs font-bold rounded-lg transition-all ${activeTab === 'dashboard' ? 'bg-white text-[#004d00] shadow-lg' : 'text-green-100 hover:bg-[#005a00] hover:text-white'}`}><Icons.Grid /> <span className="ml-3 uppercase tracking-wide">DASHBOARD</span></button>
                    <button onClick={() => { setActiveTab('profile'); setIsEditing(false); }} className={`flex items-center w-full px-5 py-3.5 text-xs font-bold rounded-lg transition-all ${activeTab === 'profile' ? 'bg-white text-[#004d00] shadow-lg' : 'text-green-100 hover:bg-[#005a00] hover:text-white'}`}><Icons.User /> <span className="ml-3 uppercase tracking-wide">MY PROFILE</span></button>
                    <button onClick={() => setActiveTab('challan')} className={`flex items-center w-full px-5 py-3.5 text-xs font-bold rounded-lg transition-all ${activeTab === 'challan' ? 'bg-white text-[#004d00] shadow-lg' : 'text-green-100 hover:bg-[#005a00] hover:text-white'}`}><Icons.File /> <span className="ml-3 uppercase tracking-wide">EXAM CHALLAN</span></button>
                </nav>
            ...
            <main className="flex-1 flex flex-col min-h-screen">
                <header className="h-16 bg-[#004d00] flex items-center justify-between px-8 shadow-md z-10 shrink-0">
                    <h2 className="text-sm font-bold text-white uppercase tracking-wider">{activeTab.replace('_', ' ')}</h2>
                    <div className="overflow-hidden w-64 h-6 relative"><p className="absolute whitespace-nowrap text-[10px] text-yellow-300 font-bold uppercase tracking-wide animate-marquee">{siteConfig.tickerText}</p></div>
                    <span className="text-[10px] text-green-200 font-bold uppercase tracking-wide">{currentDate}</span>
                </header>
                <div className="flex-1 p-8 relative">
                    {activeTab === 'dashboard' && (<><div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8"><div className="bg-white p-6 rounded-lg shadow-sm flex items-center justify-between border border-gray-100"><div><p className="text-[10px] font-bold text-gray-400 uppercase mb-1">Total Challans</p><h3 className="text-3xl font-bold text-gray-800">{total}</h3></div><div className="p-3 bg-green-50 text-green-700 rounded-lg"><Icons.File /></div></div><div className="bg-white p-6 rounded-lg shadow-sm flex items-center justify-between border border-gray-100"><div><p className="text-[10px] font-bold text-gray-400 uppercase mb-1">Pending</p><h3 className="text-3xl font-bold text-orange-500">{pending}</h3></div><div className="p-3 bg-orange-50 text-orange-500 rounded-lg"><Icons.Upload /></div></div><div className="bg-white p-6 rounded-lg shadow-sm flex items-center justify-between border border-gray-100"><div><p className="text-[10px] font-bold text-gray-400 uppercase mb-1">Verified</p><h3 className="text-3xl font-bold text-green-500">{verified}</h3></div><div className="p-3 bg-green-50 text-green-600 rounded-lg"><Icons.Eye /></div></div></div>
                    {/* SLIDE SHOW (Google Drive Image Support) */}
                    ...
                    <div className="bg-white rounded-lg shadow-sm border border-gray-200"><div className="px-6 py-4 border-b border-gray-100"><h3 className="text-xs font-bold text-gray-700 uppercase tracking-wide">Recent Activity</h3></div>{(!userData?.challans || userData.challans.length === 0) ? (<div className="p-8 text-center text-gray-400 text-sm">No recent activity found.</div>) : (<div className="divide-y divide-gray-50">{userData.challans.map((challan) => (<div key={challan.id} className="grid grid-cols-12 px-6 py-5 items-center hover:bg-gray-50 transition"><div className="col-span-5"><h4 className="text-xs font-bold text-gray-800 uppercase">{challan.part} EXAM FEE</h4><p className="text-[10px] text-gray-400 mt-1 uppercase font-mono">{challan.challanNo} | {challan.statusType} | {challan.batch}</p></div><div className="col-span-2"><span className="text-xs font-bold text-gray-700">Rs.{challan.amount}</span></div><div className="col-span-2">{challan.status === 'Verified' ? (<span className="bg-green-100 text-green-700 text-[10px] font-bold px-2 py-1 rounded uppercase">Verified</span>) : challan.status === 'Pending Verification' ? (<span className="bg-orange-100 text-orange-600 text-[10px] font-bold px-2 py-1 rounded uppercase">Pending</span>) : (<span className="bg-red-100 text-red-600 text-[10px] font-bold px-2 py-1 rounded uppercase">Not Paid</span>)}</div><div className="col-span-3 flex items-center gap-4"><button onClick={() => downloadPDF(challan, userData.profile)} className="flex items-center text-[10px] font-bold text-gray-500 hover:text-[#004d00]"><Icons.Download /> <span className="ml-1">PDF</span></button>{challan.status === 'Verified' || challan.status === 'Pending Verification' ? (<span className="text-[10px] font-bold text-gray-400 flex items-center"><Icons.Eye /> Sent</span>) : (<button onClick={() => openUploadModal(challan.id)} className="flex items-center bg-[#004d00] text-white px-3 py-1.5 rounded text-[10px] font-bold hover:bg-green-900 transition shadow-sm"><Icons.Upload /> <span className="ml-2 uppercase tracking-wide">UPLOAD</span></button>)}</div></div>))}</div>)}</div></>)}
